...
def volume_min(context: VAContext, text: str) -> None:
	device.EndpointVolume.SetMasterVolumeLevelScalar(
		context.get_options(__name__)["min_volume"] / 100, 
		None)

def volume_max(context: VAContext, text: str) -> None:
	device.EndpointVolume.SetMasterVolumeLevelScalar(
		context.get_options(__name__)["max_volume"] / 100, 
		None)

def mute(context: VAContext, text: str) -> None:
	device.EndpointVolume.SetMute(1, None)

def unmute(context: VAContext, text: str) -> None:
	device.EndpointVolume.SetMute(0, None)

def set_volume(context: VAContext, text: str) -> None:
...
def volume_up(context: VAContext, text: str) -> None:
	volume = get_volume_from_text(context, text)
	if volume is None:
...
def volume_down(context: VAContext, text: str) -> None:
	volume = get_volume_from_text(context, text)
	if volume is None:
		volume = context.get_options(__name__)["volume_step"] / 100
...
def get_volume_from_text(context: VAContext, text: str) -> int | None:
